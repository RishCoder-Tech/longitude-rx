...
// Icon mapping for Contentful icon strings
...
export default function CaseStudiesClient({ caseStudies }: { caseStudies: any[] }) {
  // Fallback data if no Contentful data is available
  ...
  const displayCaseStudies = caseStudies && caseStudies.length > 0 ? caseStudies : fallbackCaseStudies

  // Helper to get image URL from Contentful asset
  const getImageUrl = (study: any) => {
    if (study.fields?.image?.fields?.file?.url) {
      // Contentful asset URLs may not have protocol
      const url = study.fields.image.fields.file.url;
      return url.startsWith('http') ? url : `https:${url}`;
    }
    return study.image;
  };

  // Helper to get metrics from Contentful or fallback
  const getMetrics = (study: any) => {
    if (study.fields?.metrics) {
      // Contentful may store metrics as an array of objects or as a JSON string
      if (Array.isArray(study.fields.metrics)) {
        return study.fields.metrics.map((metric: any) => {
          // If Contentful stores as object with fields
          if (metric.fields) {
            return {
              label: metric.fields.label || '',
              value: metric.fields.value || '',
              icon: iconMap[metric.fields.icon] || TrendingUp,
            };
          }
          // If Contentful stores as plain object
          return {
            label: metric.label || '',
            value: metric.value || '',
      ...
      try {
        const arr = JSON.parse(study.fields.metrics);
        if (Array.isArray(arr)) {
          return arr.map((metric: any) => ({
            label: metric.label || '',
            value: metric.value || '',
            icon: iconMap[metric.icon] || TrendingUp,
          }));
                  ...
                  <div className="grid md:grid-cols-2 gap-8 p-8">
                    <div className="relative h-64 md:h-full rounded-xl overflow-hidden">
                      <Image
                        ...
                        alt={study.fields?.title || study.title}
                        fill
                      ...
                      <div className="absolute bottom-4 left-4">
                        <span className="inline-flex items-center rounded-full bg-white/90 px-4 py-1 text-sm font-medium text-admiral-900 backdrop-blur-sm">
                          {study.fields?.tag || study.tag}
                        </span>
                      </div>
                    </div>
                    <div className="space-y-6">
                      <div>
                        <h3 className="text-2xl font-bold font-outfit mb-2 text-admiral-900">
                          {study.fields?.title || study.title}
                        </h3>
                        <p className="text-admiral-600 font-semibold">
                          {study.fields?.organization || study.organization}
                        </p>
                      </div>
                      <p className="text-admiral-600 leading-relaxed">
                        {study.fields?.description || study.description}
                      </p>
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                        {getMetrics(study).map((metric: any, metricIndex: number) => (
                          <div key={metric.label || metricIndex} className="space-y-2">
                            <div className="w-10 h-10 bg-gradient-to-br from-admiral-100 to-admiral-50 rounded-lg flex items-center justify-center">
                              {metric.icon ? (
                                ...
                                <TrendingUp className="h-5 w-5 text-admiral-600" />
                              )}
                            </div>
                            <div>
